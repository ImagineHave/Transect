...
class EditForm(FlaskForm):
    ...
    payer_account = SelectField(label='pay from account', choices=get_accounts_as_list_of_tuples())
    payee = StringField('payee', [has_payee])
    payee_account = SelectField(label='pay to account', choices=get_accounts_as_list_of_tuples())
