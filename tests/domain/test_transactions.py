...
from transect.domain.transactions import insert_transaction, get_transactions, update_transaction, delete_transaction, \
    get_transactions_for_username, get_transactions_for_user_id, get_transaction, Transactions


def create_transactions(payer='PAYER', payee='PAYEE', amount=100.0, date='1982-05-14', count=1):
    transactions = []
    for i in range(count):
        dt = datetime.strptime(date, '%Y-%m-%d') + relativedelta(months=i)
        t = {'payer': payer, 'payee': payee, 'amount': amount+(10*i), 'date': dt.date()}
...
def test_update_transaction(app, test_user):
    with app.app_context():
        ...
        t = Transactions.objects(**{"user": ObjectId(user_id1), 'payer': 'PAYER'}).first()
        tid = t.get_id()

        t4 = create_transactions(payer='Z', payee='X')[0]
