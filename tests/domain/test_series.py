...
from transect.domain.series import create_transactions, insert_series, get_series_by_id, delete_series
...
def test_insert_series(app):
    with app.app_context():
        ...
        series = insert_series(username, payer, payee, amount, start_date, end_date, frequency)
        assert 25 == get_transactions_for_username(username).count()
