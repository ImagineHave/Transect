...
def test_adding_accounts(client, app, auth, test_user):
    with app.app_context():
        ...
        for account in a1s:
            ...
            assert 1 == len(get_accounts(username1, {'account_opened_date': dt}))
            assert account['account_opened_date'] == get_accounts(username1,
...
def test_editing_accounts(client, app, auth, test_user):
    with app.app_context():
        ...
        for account in a1s:
            ...
            assert account['account_name'] == get_accounts(username1, a1).first().account_name
